...
}elseif ($id==="delete" && $extra>0) 
{
	...
	if($q!='')			
	{			
		$total_searched_prescriptions = $prescription->CountSearchedPrescriptions($q,$field);	
		...
		$all_prescriptions = $prescription->GetSearchedPrescriptions($q, $field);
		...
		$total_prescriptions = $prescription->TotalPrescriptions();
		...
		if (isset($id) && $id > 0 && is_numeric($id)) {
			...
			$all_prescriptions = $prescription->ListOfPrescriptionsForSinglePatient($firstLimit,PAGINATION,$id);
			 ...
			 $all_prescriptions = $prescription->GetAllPrescriptions($firstLimit,PAGINATION);
		}
