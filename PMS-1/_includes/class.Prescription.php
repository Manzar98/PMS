...
Class Prescription
{
	...
	function GetAllPrescriptions($startIndex,$limit)
	{
		global $db;
		$sql = '
	...
	function CountSearchedPrescriptions($q,$field)
	{
		global $db;
		...
		if($field=="id" || $field=="complain" || $field=="created_on")
		{
			if($field=="id")
			{
				$sql = 'SELECT COUNT(p.id) FROM '.DB_PREFIX.'prescription p WHERE p.id='.$q.' LIMIT 1';
			}
			elseif($field=="created_on")
			{
				$sql = 'SELECT COUNT(p.id) FROM '.DB_PREFIX.'prescription p WHERE p.created_on = '.$q.' LIMIT 1';
			}
			else {
				$sql = 'SELECT COUNT(p.id) FROM '.DB_PREFIX.'prescription p WHERE '.$field.' LIKE "%'.$q.'%" LIMIT 1';			
			}
		}
	...
	function GetSearchedPrescriptions($q,$field)
	{
		global $db;
		...
		if($field=="id" || $field=="complain" || $field=="created_on")
		{
			if($field=="id")
			{
				...
				FROM  '.DB_PREFIX.'prescription p 
				LEFT JOIN '.DB_PREFIX.'patient pt ON p.patient_id = pt.id  WHERE p.id='.$q.'';
			}
			elseif($field=="created_on")
			{
				$sql = 'SELECT pt.id as patient_id,pt.name as patient_name, p.id as id, p.description as description, p.complain as complain, 
				p.next_date as next_date, p.created_on as created_on, p.updated_on as updated_on
				FROM  '.DB_PREFIX.'prescription p 
				LEFT JOIN '.DB_PREFIX.'patient pt ON p.patient_id = pt.idWHERE p.created_on = '.$q.'';
			}
			else {
				$sql = 'SELECT pt.id as patient_id,pt.name as patient_name, p.id as id, p.description as description, p.complain as complain, 
				p.next_date as next_date, p.created_on as created_on, p.updated_on as updated_on
				FROM  '.DB_PREFIX.'prescription p 
				LEFT JOIN '.DB_PREFIX.'patient pt ON p.patient_id = pt.id WHERE '.$field.' LIKE "%'.$q.'%"';			
			}
		}
		elseif($field=="patient_name") {
			$sql = '
	...
	function AddPrescription($data)
	{
		...
		if (isset($data['id'])) {
		 ...
		 (patient_id,description,complain,complain_detail,next_plan,next_date,fee_received,created_on,updated_on)
		 VALUES(
		 "'.$data["patient_id"].'",
		 "'.$data["description"].'",
		 "'.$data["complain"].'",
