...
Class Test
{
	function AddTest($data)
	 ...
	 function AddTestOptions($data,$test_id)
	 {
	  ...
	  {
		global $db;
		$sql = 'SELECT * FROM '.DB_PREFIX.'tests as tests, '.DB_PREFIX.'test_options as test_options WHERE tests.id=test_options.test_id ORDER BY tests.name ASC LIMIT '.$startIndex.', '.$limit;
		// echo $sql;
	  ...
	  function GetTestsBasic($startIndex = 0,$limit = false)
	  {
		...
		$sql = 'SELECT * FROM '.DB_PREFIX.'tests ORDER BY name ASC '.$sql_limit;
		// echo $sql;
		return $db->QueryArray($sql);
	  }
	 ...
	 {
	 	global $db;
		$sql = 'SELECT COUNT(id) FROM '.DB_PREFIX.'tests LIMIT 1';
	 ...
	 function CountTestOptions($test_id)
	 {
	  ...
	  function GetTestDetails($id)
	   ...
	   {
		 global $db;
		 $sql = 'SELECT * FROM '.DB_PREFIX.'tests WHERE id='.$id.' LIMIT 1';
		 return $db->QueryRow($sql);
	   }
		...
		function GetTestsBasiconrealtime($insertedId)
	  {
		...
		$sqlSelect = 'SELECT * FROM '.DB_PREFIX.'tests ORDER BY name ASC ';
		// echo $sql;
