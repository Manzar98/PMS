(function() {
	SIKBase.links = function() {
		var linkTemplate = [
			'<div class="linkItem">',
				...
				'<div class="linkWrapper">',
					...
					'<label><span>Name:</span><input type="text" size="60" name="name[]" value="For example: gowireless" /><img class="help" src="../../_templates/img/information-balloon.png" title="The text you want to display in the frontend" /></label>',
					'<a href="#" title="Save changes" class="saveLink"><img src="',
					'',
					'_templates/img/disk.png" alt="Save" /></a>',
					'<label><span>URL:</span><input type="text" size="60" name="url[]" value="For example: http://www.ugowireless.biz/" /><img class="help" src="../../_templates/img/information-balloon.png" title="The URL you want to link to. When you want to link to a page (e.g. the page Contact) you can use the URL (e.g. contact)" /></label>',
					'<label><span>Title:</span><input type="text" size="60" name="title[]" value="For example: Go Wireless Pakistan" /><img class="help" src="../../_templates/img/information-balloon.png" title="Text that shows when you hover over the link, just like this help message. You can leave this blank if you don\'t want to show anything" /></label>',
				'</div>',
			'</div>'
		];
		var pointerPosition;
		...
		var mouseUp = function(e) {
			...
			if (currentEl.nextSibling != elOrigNext ||  currentEl.previousSibling != elOrigPrev) {
				SIKBase.messages.add('Link order changed. Saving ...');
				saving = true;
				SIKBase.overlay.show(document.getElementById('linksContainer'));
				var order = [];
		...
		var mouseClick = function(e) {
			...
			if ($(el).is('a.deleteLink')) {
					...
					$.ajax({
						...
						success: function(responseText) {
							if (typeof responseText == 'string' && responseText.length > 0) {
								...
								SIKBase.messages.add('Link has been deleted');
							}
						},
						complete: function(){
							SIKBase.overlay.hide();
				...
				$.ajax({
					...
					success: function() {
						SIKBase.messages.add('Link has been saved');
						$(el).hide();
					},
					complete: function(){
						SIKBase.overlay.hide();
		...
		var add = function(e) {
			...
			linkTemplate[4] = SIK.SIK_admin_url;
			linkTemplate[8] = SIK.SIK_admin_url;
	...
	SIKBase.register(SIKBase.links.init, 'init');
})();