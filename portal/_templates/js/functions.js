	...
	SIK = {
		...
		FixPng: function()
		{
			...
			if ((version >= 5.5) && (document.body.filters)) 
			{
			   for(var i=0; i<document.images.length; i++)
			   {
			      ...
			      if (imgName == this.SIK_admin_url.toUpperCase() + "IMG/BT-RSS.PNG")
			      {
		...
		HandleLocationOutsideRo: function()
		{
			if (document.getElementById("location_outside_ro").style.display == "none")
			{
				...
				$("a#other_location_label").html(SIK.I18n.js.location_pick_from_list);
			}
			else
			{
				document.getElementById("city_id").removeAttribute("disabled");
				$("div#location_outside_ro").hide();
				$("a#other_location_label").html(SIK.I18n.js.location_other);
			}
		},
		DeactivateLink: function(job_id)
		{
			var url = SIK.SIK_admin_url+'deactivate/';
			SIK.Deactivate(url, job_id);
		},
		ActivateLink: function(job_id)
		{
			var url = SIK.SIK_admin_url+'activate/';
			SIK.Activate(url, job_id, 0);
		},
			   	...
			   	if (msg != "0")
					{
						...
						else
						{
							...
							linkElement.innerHTML = '<img src="'+SIK.SIK_admin_url+'_templates/img/switch.png" alt="deactivate" />';
			   	...
			   	if (msg != "0")
					{
						...
						linkElement.innerHTML = '<img src="'+SIK.SIK_admin_url+'_templates/img/switch_off.png" alt="activate" />';
        ...
        {
            var url = SIK.SIK_admin_url+'deactivate-spotlight/';
            SIK.SpotlightDeactivate(url, job_id);
        },
        ActivateSpotlight: function(job_id)
        {
            var url = SIK.SIK_admin_url+'activate-spotlight/';
            SIK.SpotlightActivate(url, job_id);
        },
                   ...
                   if (msg != "0")
                    {
						...
						{
							SIK.DeactivateSpotlight(job_id);
						};
                        ...
                        linkElement.innerHTML = '<img src="'+SIK.SIK_admin_url+'_templates/img/star_on.png" alt="deactivate" />';
                   ...
                   if (msg != "0")
                    {
                        ...
                        linkElement.innerHTML = '<img src="'+SIK.SIK_admin_url+'_templates/img/star_off.png" alt="activate" />';
		...
		{
			if(confirm(SIK.I18n.js.delete_job_confirmation_question))
			{
				$.ajax({
				  type: "POST",
				  url: url,
