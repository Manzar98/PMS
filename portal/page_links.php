<?php
	...
	if ($id == 'home' || $id == 'secondary' || $id == 'footer1' || $id == 'footer2' || $id == 'footer3') $menu = $id;
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST' && key_exists('action', $_POST)) {
		switch ($_POST['action']) {
			case 'saveOrder':
				...
				break;
			case 'newLink':
				if ($id == 'home' || $id == 'secondary' || $id == 'footer1' || $id == 'footer2' || $id == 'footer3')	$menu_id = $id; else $menu_id = 'home'; 
	...
	if (isset($menu) && $menu != '0')
	{
		...
		if ($menu == 'home') $menu_name = 'Home Links';
		elseif ($menu == 'secondary') $menu_name = 'Main Menu';
